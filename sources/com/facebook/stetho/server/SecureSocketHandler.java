...
public abstract class SecureSocketHandler implements SocketHandler {
    ...
    public static void enforcePermission(Context context, LocalSocket localSocket) {
        ...
        if (LogUtil.isLoggable(2)) {
            LogUtil.m9814v("Got request from uid=%d, pid=%d", Integer.valueOf(uid), Integer.valueOf(pid));
        }
    ...
    public final void onAccepted(LocalSocket localSocket) {
        try {
            ...
            LogUtil.m9805e("Unauthorized request: " + e.getMessage());
        }
    }

    public abstract void onSecured(LocalSocket localSocket);
}
