package com.hisense.connect_life.app_account.activity;

import android.view.View;
import com.hisense.connect_life.core.extentions.JuConnectExtKt;
import com.hisense.connect_life.core.global.EventBusConstKt;
import com.hisense.connect_life.core.global.Paths;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function1;
import kotlin.jvm.internal.Lambda;
import org.jetbrains.annotations.Nullable;

@Metadata(mo47990d1 = {"\u0000\u000e\n\u0000\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\u0010\u0000\u001a\u00020\u00012\b\u0010\u0002\u001a\u0004\u0018\u00010\u0003H\n¢\u0006\u0002\b\u0004"}, mo47991d2 = {"<anonymous>", "", "it", "Landroid/view/View;", "invoke"}, mo47992k = 3, mo47993mv = {1, 6, 0}, mo47995xi = 48)
/* compiled from: PrivacyPolicyActivity.kt */
public final class PrivacyPolicyActivity$setListeners$1 extends Lambda implements Function1<View, Unit> {
    public final /* synthetic */ PrivacyPolicyActivity this$0;

    /* JADX INFO: super call moved to the top of the method (can break code semantics) */
    public PrivacyPolicyActivity$setListeners$1(PrivacyPolicyActivity privacyPolicyActivity) {
        super(1);
        this.this$0 = privacyPolicyActivity;
    }

    public /* bridge */ /* synthetic */ Object invoke(Object obj) {
        invoke((View) obj);
        return Unit.INSTANCE;
    }

    public final void invoke(@Nullable View view) {
        String origin = this.this$0.getOrigin();
        int hashCode = origin.hashCode();
        if (hashCode != -746733270) {
            if (hashCode != -690213213) {
                if (hashCode == 103149417 && origin.equals(EventBusConstKt.LOGIN)) {
                    this.this$0.showLoading();
                    this.this$0.updateJHLUserRegisterProtocol();
                }
            } else if (origin.equals(EventBusConstKt.REGISTER)) {
                JuConnectExtKt.navigation(Paths.Account.RegisterInputEmaliActiviy);
            }
        } else if (origin.equals(EventBusConstKt.THIRD_REGISTER)) {
            if (this.this$0.getLoginEmail().length() > 0) {
                PrivacyPolicyActivity privacyPolicyActivity = this.this$0;
                privacyPolicyActivity.checkLoginNameSign(privacyPolicyActivity.getLoginEmail());
                return;
            }
            this.this$0.gotoThirdRegister();
        }
    }
}
