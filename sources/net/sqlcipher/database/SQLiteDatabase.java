...
public class SQLiteDatabase extends SQLiteClosable implements SupportSQLiteDatabase {
    ...
    private void checkLockHoldTime() {
        ...
        int i = (j > ItemTouchHelper.Callback.DRAG_SCROLL_ACCELERATION_LIMIT_TIME_MS ? 1 : (j == ItemTouchHelper.Callback.DRAG_SCROLL_ACCELERATION_LIMIT_TIME_MS ? 0 : -1));
        if ((i >= 0 || Log.isLoggable(TAG, 2) || elapsedRealtime - this.mLastLockMessageTime >= 20000) && j > 300) {
            int threadCpuTimeNanos = (int) ((Debug.threadCpuTimeNanos() - this.mLockAcquiredThreadTime) / EventLoop_commonKt.MS_TO_NS);
            if (threadCpuTimeNanos > 100 || i > 0) {
                this.mLastLockMessageTime = elapsedRealtime;
                "lock held on " + this.mPath + " for " + j + "ms. Thread time was " + threadCpuTimeNanos + LanguageConstKt.f15959ms;
    ...
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public static long getPragmaVal(net.sqlcipher.database.SQLiteDatabase r4, java.lang.String r5) {
        /*
    ...
    /* Code decompiled incorrectly, please refer to instructions dump. */
    public static void loadICUData(android.content.Context r5, java.io.File r6) {
        /*
        ...
        if (isOpen()) {
                ...
                try {
                    ...
                    if (this.mSlowQueryThreshold != -1) {
                        ...
                        if (currentTimeMillis >= ((long) this.mSlowQueryThreshold)) {
                            "query (" + currentTimeMillis + " ms): " + sQLiteDirectCursorDriver.toString() + ", args are <redacted>, count is " + -1;
                        }
                    }
                    throw th;
                }
            ...
            if (this.mSlowQueryThreshold != -1) {
                ...
                if (currentTimeMillis2 >= ((long) this.mSlowQueryThreshold)) {
                    "query (" + currentTimeMillis2 + " ms): " + sQLiteDirectCursorDriver.toString() + ", args are <redacted>, count is " + i;
                }
            }
        ...
        if (isOpen()) {
            ...
            try {
                ...
                if (this.mSlowQueryThreshold != -1) {
                    ...
                    long currentTimeMillis = System.currentTimeMillis() - j;
                    if (currentTimeMillis >= ((long) this.mSlowQueryThreshold)) {
                        "query (" + currentTimeMillis + " ms): " + sQLiteDirectCursorDriver.toString() + ", args are <redacted>, count is " + i;
                    }
                }
                return new CrossProcessCursorWrapper(query);
            } catch (Throwable th) {
                if (this.mSlowQueryThreshold != -1) {
                    long currentTimeMillis2 = System.currentTimeMillis() - j;
                    if (currentTimeMillis2 >= ((long) this.mSlowQueryThreshold)) {
                        "query (" + currentTimeMillis2 + " ms): " + sQLiteDirectCursorDriver.toString() + ", args are <redacted>, count is " + -1;
                    }
                }
                throw th;
            }
